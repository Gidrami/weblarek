...
export class CardPreviewView extends Component<IProduct> {
	...
	constructor(
		...
		this.basketButton.addEventListener('click', (e) => {
			...
			this.events.emit(appEvents.BASKET_ADD, { product: this.state })
		})
	}
	...
	protected setValues(): void {
		...
		const cardText = this.container.querySelector(
			'.card__text',
		) as HTMLElement
		...
		this.basketButton.textContent = withoutPrice ? 'Недоступно' : 'В корзину'
	}
}
